...
// Теперь создадим функцию, которая будет менять цвет фона при нажатии на карточку из колекции (но при нажатиии именно на вложенный div),
...
// dataset - это свойство объекта элемента, которое позволяет получить доступ к пользовательским атрибутам данных(data -) элемента в виде объекта DOMStringMap. 
// Атрибуты данных(data -) используются для хранения информации, которая не отображается на экране, но может быть полезна для JavaScript или CSS.
...
// // Для этого в современном JS есть классный метод: closest который находит ближайший элемент с классом(либо селектором) указанным в ():
...
// // карточек активными, это потому что мы добавляем класс, но не удаляем. Соответсвенно необходимо добавить воможность снятия класса 'is-active'
...
//   document.body.style.backgroundColor = color; // где color это выражение swatchEl.target.dataset.hex; которое мы передаем в парамер функции 
// }
